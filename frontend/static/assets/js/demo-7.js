...
(function($) {
    ...
    $("body").on("click", ".add-to-cart", function(){
        if ($(this).hasClass("js-one-click-add")) {
            return;
        }
    ...
    (function() {
        ...
        $("body").on("click", ".ec-pro-content .remove", function(){
            if ($(this).hasClass("js-side-cart-remove")) {
                return;
            }
        ...
        // $(".ec-pro-content .remove").on("click", function () {
            ...
            var count = parseInt($(".ec-cart-count").html(), 10) || 0;
            count = Math.max(0, count - 1);
            $(".ec-cart-count, .cart-count-lable").html(count);
    ...
    /*--------------------- Quick view Slider ------------------------------ */
    if ($.fn.slick && $('.qty-product-cover').length && $('.qty-nav-thumb').length) {
        $('.qty-product-cover').each(function () {
            var $slider = $(this);
    ...
    $(".qty-plus-minus").each(function() {
        var $qtyWrapper = $(this);

        if (!$qtyWrapper.children(".dec.ec_qtybtn").length) {
            $qtyWrapper.prepend('<div class="dec ec_qtybtn">-</div>');
        }

        if (!$qtyWrapper.children(".inc.ec_qtybtn").length) {
            $qtyWrapper.append('<div class="inc ec_qtybtn">+</div>');
        }
    });

    $("body").off("click.ecQty", ".qty-plus-minus .ec_qtybtn").on("click.ecQty", ".qty-plus-minus .ec_qtybtn", function() {
        var $qtybutton = $(this);
        var $qtyWrapper = $qtybutton.closest(".qty-plus-minus");
        var QtyoldValue = parseFloat($qtyWrapper.find("input").val()) || 1;
        var QtynewVal = QtyoldValue;

        if ($qtybutton.hasClass("inc") || $qtybutton.text() === "+") {
            QtynewVal = QtyoldValue + 1;
        } else if (QtyoldValue > 1) {
            QtynewVal = QtyoldValue - 1;
        }

        $qtyWrapper.find("input").val(QtynewVal);
    });
    ...
    /*--------------------- Category Slider -------------------------------- */    
    if ($.fn.slick && $('#ec-cat-slider').length && !$('#ec-cat-slider').hasClass('slick-initialized')) {
        $('#ec-cat-slider').slick({
            rows: 1,
            dots: false,
            arrows: true,
...
/*--------------------- Trend Product Slider -------------------------------- */    
    ...
    /*--------------------- Add To  Cart Toast -------------------------------- */    
    $(document).ready(function(){
        $(".ec-btn-group.add-to-cart").not(".js-one-click-add").click(function() {
   ...
   /*--------------------- BG Skin ---------------------- */
    ...
    /*----------------------------- Sidebar Toggle (Animation Check Fix) -------------------------------- */
$(window).on('load', function() {
    
    // 1. Force removal of existing handlers one last time
    $(document).off('click', '.ec-sidebar-title'); 
    $(".ec-sidebar-title").off('click').unbind('click');

    // 2. Add the new "Smart" Handler
    $("body").on("click", ".ec-sidebar-title", function(e) {
        
        // Prevent default browser jumps
        e.preventDefault();
        e.stopPropagation();

        var $content = $(this).closest('.ec-sidebar-block').find('.ec-sb-block-content');

        // --- THE FIX ---
        // If the menu is currently moving (opening or closing), STOP.
        // This ignores the accidental "double click" from conflicting scripts.
