...
(function ($) {
    ...
    $(window).on("scroll", function () {
        var $stickyNextSection = $('.sticky-header-next-sec'),
            $window = $(window);

        if (!$stickyNextSection.length) {
            checkScroll();
            return;
        }

        var distance = $stickyNextSection.offset().top;
    ...
    (function () {
        ...
        $("body").on("click", ".ec-pro-content .remove", function () {
            ...
            var count = parseInt($(".cart-count-lable").html(), 10) || 0;
            count = Math.max(0, count - 1);
            $(".cart-count-lable, .ec-cart-count").html(count);
